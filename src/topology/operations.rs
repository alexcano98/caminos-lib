...
For the same concept on patterns see [RemappedNodes](crate::general_pattern::RemappedNodes).
...
impl RemappedServersTopology
{
	pub fn new(mut arg:TopologyBuilderArgument) -> RemappedServersTopology
	{
		...
		let mut pattern = pattern.expect("There were no general_pattern in configuration of RemappedServersTopology.");
...
/**
Deletes `amount` links selected randomly. May employ a general_pattern to select on what switches they fault may occur.
...
impl RandomLinkFaults
{
	pub fn new(mut arg:TopologyBuilderArgument) -> RandomLinkFaults
	{
		...
		match_object_panic!(arg.cv, "RandomLinkFaults", value,
			...
			"switch_pattern" => switch_pattern = Some(new_pattern(GeneralPatternBuilderArgument{cv:value,plugs:arg.plugs})),
